...
%sampling frequency
fs = 250;
%extracting and experimenting with 6th trial. 
...
print(fig1, '-djpeg','T6_TS');
%trial 6 power signal density
savefig(fig2);
print(fig2, '-djpeg','T6_PSD');
%% Filtering
...
subplot(2,1,1);
pwelch(Channel_O2_trial,[],[],[],fs);
title('PSD-Original O2 Signal');
subplot(2,1,2);
pwelch(filtered_O2,[],[],[],fs);
title('PSD-Chebyshev Filtered O2 Signal');
savefig(fig4);
print(fig4, '-djpeg','PSD original VS chebytchev type2');
...
for i=1:length(t)
    %j is the number of electrodes/trials
    for j=1:23
...
savefig(fig6);
print(fig6, '-djpeg','O2 channel - CAR and Chebyschev type2 applied');
...
X(:,2) = [];
Y = cell(size(X,1), 1);
Y(1:size(burning_feat_mat, 1)) = {'exploding'};
